...
Please also read the link:CODE_OF_CONDUCT.md[Code of Conduct], the
link:GOVERNANCE.adoc[Governance model], and the
link:SECURITY.md[Security policy].

=== Ground truth first

This project’s headline value is _honesty about coverage_. The same
...
[arabic]
. Fork and create a topic branch (`+fix/...+`, `+feat/...+`,
`+docs/...+`).
. Keep changes focused; one logical change per PR.
. Ensure every source file carries an SPDX header (`+MPL-2.0+` for code,
`+CC-BY-SA-4.0+` for prose).
. Run the relevant checks locally (`+just prove+`, `+just abi-check+`,
`+cargo test+`, `+zig build test+`).
. Sign your commits (SSH or GPG).
. Follow https://www.conventionalcommits.org/[Conventional Commits].

==== Licensing of contributions

By contributing you agree that your contributions are licensed under the
project’s terms: *MPL-2.0* for code, *CC-BY-SA-4.0* for documentation
and prose.
